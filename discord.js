...
class Music {
    ...
    async play(msg) {
        ...
        if (!this.connection[guildID]) {
            msg.channel.send('請先用 `!!join` 讓我加入頻道OuO');
            return;
        }

        // 如果 Bot leave 後又未加入語音頻道
        if (this.connection[guildID].status === 4) {
            msg.channel.send('請用 `!!join` 讓我重新加入頻道OuO');
            return;
        }
        ...
        try {
            ...
            if (this.isPlaying[guildID]) {
                msg.channel.send(`放入播放清單：${info.title}`);
            } else {
